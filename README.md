...
#### *Note*:  Using large jsonl files may cause host out of memory issues. As a solution, dataset tokenization can be done on multiple files, and the generated tokenized files can be merged later in Step 4. This solution can be used with any tokenization method. As an example, it is shown for GPT2BPETokenizer method.
*  Tokenize the dataset using GPT2BPETokenizer:
    ...
    ```
   * The below example shows the dataset tokenization of a single file:
      ```bash
      ...
      ```
   * The below example shows the dataset tokenization of multiple files:
      ```bash
  ...
  * Tokenize the dataset using HFTokenizer:
    ...
    ```
# Step 4 :
 * This step is required when multiple tokenized dataset files are generated and merged into a single file using the below method:
    ```bash
